...
const BarChart = ({
    ...
    const updateVis = () => {
        ...
        .on('click', function(event, d) {
            ...
            let index;
            if (d.key == 'Easy') {
                activeCategory[0] = 1 - activeCategory[0];
                index = 0;
            }
            else if (d.key == 'Intermediate') {
                activeCategory[1] = 1 - activeCategory[1];
                index = 1;
            }
            else {
                activeCategory[2] = 1 - activeCategory[2];
                index = 2;
            }
            ...
            for (let i = 0; i < 3; i++) {
                if (activeCategory[i]) {
                    activeKey.push(orderedKeys[i]);
                }
            }
            //if no category is active, display all the category
            if (activeCategory[0] == 0 && activeCategory[1] == 0 && activeCategory[2] == 0 ) {
                activeKey = ['Easy','Intermediate','Difficult'];
            }
            
            // Change parent node's React State with the selected category names (5pts)

            // To DO
            setFilterCategory(activeKey);
        });
